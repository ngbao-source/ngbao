...
# === THÊM HÀM LẤY DANH SÁCH BOX TỪ COOKIE (từ code bạn gửi) ===
...
def spam_call_group(cookie, thread_id, times=50, delay=2.5):
    uid = get_uid(cookie)
    if not uid:
        print(f"[red][COOKIE {uid or '???'}] → Không lấy được UID → bỏ qua[/]")
        return

    print(f"[bold yellow][CALL] → Bắt đầu réo nhóm {thread_id} | {times} lần | delay {delay}s[/]")

    fb_dtsg = jazoest = None
    headers = {
        'cookie': cookie,
    ...
    for attempt in range(3):
        try:
            home = requests.get('https://mbasic.facebook.com', headers=headers, timeout=15).text
            fb_dtsg = re.search(r'name="fb_dtsg" value="(.*?)"', home)
            jazoest = re.search(r'name="jazoest" value="(\d+)"', home)
    ...
    if not fb_dtsg or not jazoest:
        print(f"[red][COOKIE {uid}] → Không lấy được fb_dtsg/jazoest → bỏ qua nhóm này[/]")
        return
    ...
    for i in range(times):
        try:
            raw = f"FRAYFVJPT006{str(thread_id).zfill(16)}{base64.b64encode(os.urandom(20)).decode()[:20]}"
            msg_b64 = base64.b64encode(raw.encode()).decode()

            variables = {
                "input": {
                    "endpoint": {"appId": "2220391788200892", "deviceId": device_id, "userId": uid},
                    "message": msg_b64,
                    "actor_id": uid,
                    "client_mutation_id": str(i + 100)
                }
            }
            ...
            data = {
                ...
                '__a': '1',
                '__req': hex(i + 10)[2:],
                '__hs': '20413.HYP:comet_pkg.2.1.0.0.',
                'dpr': '1',
                '__rev': '1030224246',
                '__comet_req': '15',
                'fb_dtsg': fb_dtsg,
                ...
                'fb_api_req_friendly_name': 'ZenonMWThriftSendMessageMutation',
                'variables': json.dumps(variables, separators=(",", ":")),
                'server_timestamps': 'true',
                'doc_id': '9720271894689175',
            }
            ...
            # Check thành công (Facebook trả về JSON ngắn hoặc có từ khóa)
            if r.status_code == 200 and (len(r.text) < 2500 or 'ZenonCallInviteSent' in r.text or '"error"' not in r.text.lower()):
                success += 1
                print(f"[bold green]   → Lần {i+1}/{times} | RÉO THÀNH CÔNG! (+{success})[/]")
            else:
                print(f"[red]   → Lần {i+1}/{times} | thất bại[/]")

            time.sleep(delay)

        except Exception as e:
            print(f"[red]   → Lần {i+1} lỗi: {str(e)[:50]}[/]")
            time.sleep(3)

    print(f"[bold cyan][CALL] → Nhóm {thread_id} xong! Thành công: {success}/{times}[/]")
...
class minhloi:
    def __init__(self, cookie, mqtt_broker="broker.hivemq.com", mqtt_port=1883):
        ...
        # Remove callback_api_version for older paho-mqtt versions
...
if __name__ == "__main__":
    try:
        clr()
        console.rule("[bold red]TOOL TREO MESSENGER - Nguyen hoang gia bao V4 TURBO (KHỞI ĐỘNG CÙNG LÚC)[/]")

        print("=== CHỌN CHẾ ĐỘ HOẠT ĐỘNG ===")
        print("1. Treo tin nhắn thường")
        print("2. Treo share danh bạ")
        print("3. Treo share link")
        print("4. Nhảy tên nhóm")
        print("5. Treo ảnh/video từ URL")
        print("6. Reg box")
        print("7. Treo poll")
        print("8. Fake typing + nhắn tin")
        print("9. Đổi theme nhóm")
        print("10. Đổi biệt danh tất cả thành viên")
        print("11. Spam call nhóm")

        option = IntPrompt.ask("\n[bold yellow][+] Chọn chế độ (1-11)", default=1)
        if option not in range(1, 12):
            print("[red]Chế độ không hợp lệ![/]")
            exit()

        cookie_file = Prompt.ask("[bold yellow][+] File cookie (.txt)", default="cookie.txt")
        if not os.path.isfile(cookie_file):
            print("[red]File cookie không tồn tại![/]")
            exit()

        cookies = read_cookies_from_file(cookie_file)
        if not cookies:
            print("[red]Không có cookie nào hợp lệ![/]")
            exit()
        print(f"[bold green]Đã load {len(cookies)} cookie – Sẵn sàng chiến![/]")

        # === LẤY DANH SÁCH BOX TỪ COOKIE ĐẦU TIÊN ===
        print("\n[cyan]Đang lấy danh sách nhóm chat...[/]")
        thread_list_result = get_thread_list(cookies[0])

        if isinstance(thread_list_result, dict) and "error" in thread_list_result:
            print(f"[red]Lỗi lấy box: {thread_list_result['error']}[/]")
            exit()
        if not thread_list_result:
            print("[red]Không lấy được nhóm nào![/]")
            exit()

        table = Table(title=f"[bold magenta]TÌM THẤY {len(thread_list_result)} NHÓM[/]")
        table.add_column("STT", style="cyan", justify="right")
        table.add_column("Tên nhóm", style="white")
        table.add_column("ID nhóm", style="yellow")
        for i, thread in enumerate(thread_list_result, 1):
            name = (thread["thread_name"][:50] + "...") if len(thread["thread_name"]) > 50 else thread["thread_name"]
            table.add_row(str(i), name, thread["thread_id"])
        console.print(table)

        # === CHỌN BOX ===
        choice = Prompt.ask("\n[bold green][+] Nhập STT nhóm cần treo (vd: 1,3,5 hoặc 1-10 hoặc all)", default="all")
        indices = set()
        if choice.strip().lower() == "all":
            indices = set(range(1, len(thread_list_result) + 1))
        else:
            for part in choice.replace(" ", "").split(","):
                if "-" in part:
                    try:
                        s, e = map(int, part.split("-"))
                        indices.update(range(max(1, s), min(len(thread_list_result), e) + 1))
                    except: pass
                else:
                    try: indices.add(int(part))
                    except: pass

        selected_threads = [thread_list_result[i-1]["thread_id"] for i in sorted(indices) if 1 <= i <= len(thread_list_result)]
        if not selected_threads:
            print("[red]Không chọn được nhóm nào![/]")
            exit()

        print(f"\n[bold red]ĐÃ CHỌN {len(selected_threads)} NHÓM ĐỂ ĐÁNH:[/]")
        for i, tid in enumerate(selected_threads, 1):
            print(f"   {i}. {tid}")

        # === CẤU HÌNH CHUNG ===
        delay = float(Prompt.ask("[bold yellow][+] Delay giữa các tin (giây)", default="3"))
        ...
        if option == 1:
            path = Prompt.ask("[+] File nội dung tin nhắn (.txt)")
            if os.path.isfile(path): message_files = [path]
            else: print("[red]File không tồn tại![/]"); exit()

        elif option in [2, 3]:
            uid_share = Prompt.ask("[+] UID share (Enter = chính acc)", default="")
            path = Prompt.ask("[+] File nội dung (.txt)")
            if os.path.isfile(path): message_files = [path]
            else: print("[red]File không tồn tại![/]"); exit()

        elif option == 4:
            name_file = Prompt.ask("[+] File tên nhóm (nhay.txt)")
            if not os.path.isfile(name_file): print("[red]File không tồn tại![/]"); exit()

        elif option == 5:
            file_url = Prompt.ask("[+] URL ảnh/video")
            txt = Prompt.ask("[+] File nội dung đi kèm (Enter nếu không)", default="")
            if txt and os.path.isfile(txt): message_files = [txt]

        elif option in [7, 8]:
            name_file = Prompt.ask("[+] File nội dung (nhay.txt)")
            if not os.path.isfile(name_file): print("[red]File không tồn tại![/]"); exit()

        # === KHỞI ĐỘNG CÙNG LÚC – TURBO MODE ===
        print(f"\n[bold red]SẴN SÀNG PHÓNG {len(cookies)} ACC VÀO {len(selected_threads)} BOX[/]")
        print(f"[bold yellow]→ Delay: {delay}s | Chế độ: {option} | Ấn Ctrl+C để dừng[/]")
        print("[bold cyan]Đếm ngược khởi động đồng loạt...[/]")

        for i in range(5, 0, -1):
            print(f"   [bold red]{i}...[/]", end="\r")
            time.sleep(1)
        print("   [bold green]GO GO GO!!! BÃO TỚI RỒI ANH EM ƠI!!![/]")

        def worker(cookie):
            uid = get_uid(cookie) or "unknown"
            hash_id = hashlib.md5(cookie.encode()).hexdigest()[:8]
            try:
                print(f"[bold green][+] ACC {uid} ({hash_id}) ĐÃ VÀO TRẬN![/]")
                send_messages_with_cookie(
                    ...
                    contact_uid=uid_share if uid_share else None,
                    name_file=name_file if option in [4, 7, 8] else None,
                )
            except Exception as e:
                print(f"[bold red][x] ACC {uid} die: {str(e)[:60]}[/]")

        # KHỞI ĐỘNG TẤT CẢ CÙNG LÚC
        threads = []
        for cookie in cookies:
            t = threading.Thread(target=worker, args=(cookie,), daemon=True)
            t.start()
            threads.append(t)

        print(f"\n[bold magenta]ĐÃ PHÓNG {len(cookies)} ACC CÙNG LÚC – ĐANG TẤN CÔNG {len(selected_threads)} BOX![/]")
        print("[yellow]Nhấn Ctrl+C để dừng toàn bộ[/]")
        ...
        try:
            ...
            print("\n[bold red]DỪNG TOOL – TẠM BIỆT ANH EM! BOX ĐÃ NÁT RỒI[/]")
            os._exit(0)

    except KeyboardInterrupt:
        print("\n[yellow]Dừng bởi người dùng[/]")
    except Exception as e:
        print(f"[red]Lỗi nghiêm trọng: {e}[/]")